...
\KwIn{last\_desired\_state, path, segment\_number, segment\_distance}
\KwOut{desired\_state, segment\_number, segment\_distance}

$current\_robot\_state$ = \GetCurrentRobotState{} \;
\eIf{path $\neq \emptyset$}
{
	
}
{
	$segment\_number$ = $0$ \; \nllabel{empty_path_start}
	$segment\_distance$ = $0.0$ \;
	$desired\_state$ = \MakeHaltState{current\_robot\_state} \; \nllabel{empty_path_end}
}

\end{algorithm}

Lines \ref{empty_path_start} to \ref{empty_path_end} of \autoref{alg:trajectory_generation_algorithm}

\todo{MakeHaltState algorithm/description}
