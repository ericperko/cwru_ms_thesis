...
Another issue with the implementation used by costmap_2d is that, with a very fine grid cell resolution, obstacles can leave "droppings" in the map. These "droppings" occur when an obstacle such as a human moves across the sensor's field of view and is not completely cleared out of the map because, when raytracing
